<template>
    <div class="scene-goods">
        <div class="steps">
            <el-steps :active="active" finish-status="success" style="margin-top: 20px" v-if="$route.query.type === 'scene_add'">
...
import { queryPurGoods, addPurGoods } from '@/api/business';

export default {
    name: 'scene-purchasing-goods',
    components: {
        Table
    },
    data() {
    ...
    computed: {
        ...
        active: vm => vm.scene.length > 3 ? 4 : 2
    },
    methods: {
        ...
        query () {
            ...
            queryPurGoods(params).then(({datas, status, message}) => {
                this.loading = false;
                let list = [];
        ...
        saveSelection() {
            return new Promise((resolve, reject) => {
                if (this.selections.length > 0) {
                    addPurGoods(obj2FormData({selections: this.selections.map(item => item.id)}), this.scene.id).then(() => {
                        this.$notify({
                            title: '成功',
                            message: '添加成功',
                            type: 'success'

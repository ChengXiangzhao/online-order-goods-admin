...
import { queryAappraisalDict, querySelectedGoods } from '@/api/business';
    ...
    methods: {
        ...
        query () {
            ...
            let params = {
                ...
                appraisal_id: this.searchForm.id
            };
            if (this.searchForm.fashion !== '') {
                params.fashion = this.searchForm.fashion;
            }
            querySelectedGoods(params).then(({datas, status, message}) => {
                this.loading = false;
                let list = [];
                datas.infos.forEach(item => {
                    item.base.thumbUrl = item.relateData.goodsData.thumbUrl;
                    item.base.status_prod = item.relateData.goodsData.status_prod;
                    item.base.fashion = item.relateData.goodsData.fashion;
                    item.base.appname = item.relateData.appraisalData.name;
                    list.push(item.base);
                });
        ...
        getAappraisalDict () {
            queryAappraisalDict({point_format_list: 'cascade'}).then((res) => {
                this.appraisals = Object.keys(res.datas).map(item => {
                    return {
                        value: item,
                        name: res.datas[item]
                    };
        ...
        handleSelectChange (val) {
            this.selectedApp = arrayFind(this.appraisals, 'value', val);
            this.query();
        },
        init(id) {
